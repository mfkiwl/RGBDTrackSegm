...
from models import create_segmnet_x4_attn
...
if __name__ == '__main__':
    ...
    start_epoch = 4
    pretrained_model = './train/segmnet_x4_attn/model_e03.pth'
    ...
    # Create model
    net = create_segmnet_x4_attn(pretrained_backbone=True)
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        for i, data in enumerate(loader_train):
            ...
            # print(pred_scores.shape) # pred_scores , B, 1, 7, 7
            pred_scores = pred_scores.squeeze()
            score_loss = torch.mean(torch.sum(torch.sum((pred_scores - test_gauss) ** 2, dim=1), dim=1))
            # score_loss = torch.sum((pred_scores - test_gauss) ** 2) / batch_size
