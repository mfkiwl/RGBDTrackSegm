...
from models import create_segmnet_x4_rgb
...
if __name__ == '__main__':
    ...
    # Create model
    net = create_segmnet_x4_rgb(pretrained_backbone=True)
    ...
    for epoch in range(n_epochs):
        ...
        # Try to not save backbone weights??
        # new_state_dict = OrderedDict()
        # state_dict = net.state_dict()
        # for key, value in state_dict.items():
        #     if not key.startswith('backbone'):
        #         new_state_dict[key] = value
        # torch.save(new_state_dict, os.path.join(output_path, 'model_e%02d.pth'%epoch))
