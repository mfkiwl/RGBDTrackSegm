...
try:
    ...
    from .query_decoder_x3 import QueryDeCoder
except:
    from resnet import Bottleneck, resnet50
    from pixel_decoder import create_pixel_decoder
    from query_decoder_x3 import QueryDeCoder
...
class SegmNet(nn.Module):
    def __init__(self, backbone, pixel_decoder, query_decoder,
                       ...
                       pixel_decoder_layers=['layer1', 'layer2', 'layer3', 'sep']):
        ...
        self.fg_query_pool0 = RoIPool((1, 1), 1/32) # for layer4, Bx2048x7x7, 1/32
        self.fg_query_pool1 = RoIPool((4, 4), 1/16) # for layer3, Bx1024x14x14, 1/16
        self.fg_query_pool2 = RoIPool((8, 8), 1/8)  # for layer2, Bx512x28x28, 1/8

        self.bg_query_pool0 = nn.AdaptiveAvgPool2d(1) #   7x7 -> 1x1
        self.bg_query_pool1 = nn.AdaptiveAvgPool2d(3) # 14x14 -> 3x3
        self.bg_query_pool2 = nn.AdaptiveAvgPool2d(3) # 28x28 -> 3x3

        self.post2 = conv(100, 32)
    ...
    def flatten_pixel_features(self, f):
        ...
        '''
        return [self.flatten_feat(self.feat_pool(f[key])) for key in self.pixel_decoder_layers]
    ...
    def init_queries(self, train_rgbd_feat, train_mask, train_rois):
        ...
        return [fg_query0, fg_query1, fg_query2], [bg_query0, bg_query1, bg_query2]
    ...
    def prediction_head(self, test_feat, fg_query, bg_query):
        ...
        #
        fg_mask = torch.matmul(fg_query, pixels) # [B, Q=1+16+64=81, C] * [B, C, Pixels] -> [B, Q, Pixels]
    ...
    def forward(self, train_color, train_depth, train_mask, train_rois, test_color, test_depth):
        ...
        # Update the queires
        fg_queries = self.query_decoder(fg_queries, test_pixel_feat) # [B, 1+9+25, 256]
        bg_queries = self.query_decoder(bg_queries, test_pixel_feat) # [B, 1+9+9, 256]
...
def create_segmnet(pretrained_backbone=False,
                   ...
                   pixel_decoder_output_layers=['layer1', 'layer2', 'layer3', 'sep']):
