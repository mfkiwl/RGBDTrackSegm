...
Code from Pytracking, https://github.com/visionml/pytracking/blob/master/ltr/models/backbone/resnet.py

Axial_Layer from https://github.com/MartinGer/Stand-Alone-Axial-Attention
'''
import math
import torch
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, dilation=1, use_bn=True,
                       attention=False, num_heads=8, kernel_size=56, inference=False, base_width=64):
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, dilation=1,
                       attention=False, num_heads=8, kernel_size=56, inference=False, base_width=64, groups=1):
        ...
        if not attention:
            ...
            self.height_block = Axial_Layer(in_channels=width, num_heads=num_heads, kernel_size=kernel_size, inference=inference)
            self.width_block = Axial_Layer(in_channels=width, num_heads=num_heads, kernel_size=kernel_size, stride=1, height_dim=False, inference=inference)
...
class ResNet(Backbone):
    ...
    def __init__(self, block, layers, output_layers, num_classes=1000, inplanes=64, dilation_factor=1, frozen_layers=(),
                       attention=[False, False, False, False], num_heads=8, width_per_group=64, image_size=224, inference=False):
        ...
        self.layer1 = self._make_layer(block, inplanes, layers[0], dilation=max(dilation_factor//8, 1),
                                       attention=attention[0], num_heads=num_heads, kernel_size=image_size//4, inference=inference)
        self.layer2 = self._make_layer(block, inplanes*2, layers[1], stride=stride[0], dilation=max(dilation_factor//4, 1),
                                       attention=attention[1], num_heads=num_heads, kernel_size=image_size//8, inference=inference)
        self.layer3 = self._make_layer(block, inplanes*4, layers[2], stride=stride[1], dilation=max(dilation_factor//2, 1),
                                       attention=attention[2], num_heads=num_heads, kernel_size=image_size//16, inference=inference)
        self.layer4 = self._make_layer(block, inplanes*8, layers[3], stride=stride[2], dilation=dilation_factor,
                                       attention=attention[3], num_heads=num_heads, kernel_size=image_size//32, inference=inference)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1,
                          attention=False, num_heads=8, kernel_size=56, inference=False):
        downsample = None
...
if __name__ == '__main__':
    ...
    model = resnet50(output_layers=['layer1', 'layer2', 'layer3', 'layer4'], attention=[False, False, False, False])
    model = model.to(device)
    optimizer = optim.SGD(model.parameters(), lr=0.01, momentum=0.9)
    loss = nn.MSELoss()

    input = torch.zeros(8, 3, 224, 224).to(device)
    label = torch.zeros(8, 2048, 7, 7).to(device)

    model.zero_grad()
    optimizer.zero_grad()

    output = model(input)
    loss = loss(output['layer4'], label)
    loss.backward()
    print(model.conv1.weight.grad)
    # layer1 torch.Size([8, 256, 56, 56])
    # layer2 torch.Size([8, 512, 28, 28])
    # layer3 torch.Size([8, 1024, 14, 14])
    # layer4 torch.Size([8, 2048, 7, 7])
