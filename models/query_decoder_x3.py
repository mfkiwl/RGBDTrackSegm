...
class QueryDeCoder(nn.Module):
    ...
    def forward(self, queries, pixel_features):
        ''' fg_queries : [B, 1, 2048], [B, 9, 1024], [B, 25, 512]
        ...
        q3 = self.downsample3(self.decoder3_2(self.decoder3_1(q3, pixel_features[2]))) # BxPatchesx512 ->  BxPatchesx256

        # FFN
        _query = self.linear2(self.dropout3(self.activation(self.linear1(q3)))) # BxQx256, Q=(1+9+25)
        query = q3 + self.dropout4(_query)
        query = self.norm3(query)
...
if __name__ == '__main__':
    #
    # model = QueryDeCoderLayer(hidden_size=2048, d_ffn=1024, dropout=0.1, activation="relu", n_heads=8)
    # model = model.cuda()
    #
    # query = torch.rand(32, 10, 2048).cuda()
    # pixel_features = torch.rand(32, 64, 2048).cuda()
    #
    # output = model(query, pixel_features)
    # print(output.shape) # [32, 10, 2048]
    #
    model = QueryDeCoder()
    query = [torch.zeros(32, 1, 2048), torch.zeros(32, 9, 1024), torch.zeros(32, 25, 512)]
    pixel_features = [torch.zeros(32, 49, 2048), torch.zeros(32, 49, 1024), torch.zeros(32, 49, 512)]
