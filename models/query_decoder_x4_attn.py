...
class Attention(nn.Module):
    ...
    def forward(self, Q, K, V):
        ...
        cluster_centers = torch.matmul(attention_scores, value_layer)           # [B, Head, Q, Head_size]
        cluster_centers = cluster_centers.permute(0, 2, 1, 3).contiguous()      # [B, Q, heads, head_size]
...
class QueryDeCoderLayer(nn.Module):
    ...
    def forward(self, query, kv=None):
        """ query : template target features
            ...
            should we exchange the order of self-attention and cross attention ???

            input -> cross_attn -> self_attn
            input -> self_attn -> cross_attn ??
        """
        # cross attention
        if kv is None:
            q2, _ = self.cross_attn(query, query, query)
            attn = None
        else:
...
class QueryDeCoder(nn.Module):
    ...
    def flatten_attn_maps(self, attn1, attn2, attn3, attn4):
        attn1 = F.relu(attn1) # [B, H, Q1, P=49]
        attn1 = attn1.view(attn1.shape[0], -1, attn1.shape[-1])
        attn1 = torch.mean(attn1, dim=1) # {B, P1}

        attn2 = F.relu(attn2)  # [B, H, Q1, P=49]
        attn2 = attn2.view(attn2.shape[0], -1, attn2.shape[-1])
        attn2 = torch.mean(attn2, dim=1) # {B, P1}

        attn3 = F.relu(attn3)  # [B, H, Q1, P=49]
        attn3 = attn3.view(attn3.shape[0], -1, attn3.shape[-1])
        attn3 = torch.mean(attn3, dim=1) # {B, P1}

        attn4 = F.relu(attn4)  # [B, H=8, Q, P=49]
        attn4 = attn4.view(attn4.shape[0], -1, attn4.shape[-1]) # [B, HQ, P]
        attn4 = torch.mean(attn4, dim=1) # [B, P=49]

        h1 = int(math.sqrt(attn1.shape[1]))
        attn1 = attn1.view(attn1.shape[0], 1, h1, h1)

        h2 = int(math.sqrt(attn2.shape[1]))
        attn2 = attn2.view(attn2.shape[0], 1, h2, h2)

        h3 = int(math.sqrt(attn3.shape[1]))
        attn3 = attn3.view(attn3.shape[0], 1, h3, h3)

        h4 = int(math.sqrt(attn4.shape[1]))
        attn4 = attn4.view(attn4.shape[0], 1, h4, h4) # Bx1x7x7

        attn1 = F.interpolate(attn1, size=(h4, h4))
        attn2 = F.interpolate(attn2, size=(h4, h4))
        attn3 = F.interpolate(attn3, size=(h4, h4))

        # scores = attn1 + attn2 + attn3 + attn4
        scores = torch.cat((attn1, attn2, attn3, attn4), dim=1) # Bx4x7x7
        # scores = scores / 4.0
        scores = torch.mean(scores, dim=1) # Bx7x7
        return scores.unsqueeze(1) # Bx1x7x7
    ...
    def forward(self, queries, pixel_features):
        ...
        scores = self.flatten_attn_maps(attn1, attn2, attn3, attn4)
